...
_Requires: Vivado 2015.4 and its settings64.sh and a JVM that can run Scala_

If you don't already have a copy of the rocket, initialize the submodule pointer to rocket chip, and its first level of submodule pointers (which include chisel, and firrtl)

    $ make init-rocket-submodule

Otherwise you may symlink your working rocket chip directory into this repository in place of the submodule or override `ROCKET_DIR` make variable whose default is set common/Makefrag
...
The configuration used to generate the rocket chip comes from the `CONFIG` environment variables. If `CONFIG` isn't set by the environment, it is taken from the `Makefile` for the current board. For this example, we use the Zybo which has a default configuration of `DefaultFPGASmallConfig`.

#### <a name="genRC"></a> Generating Verilog for Rocket Chip

Enter into the directory for your board (current options are `zybo`, `zedboard`, and `zc706`). After making changes within `rocket-chip` and/or `common/src/main/scala`, to run the rocket chip generator and copy the newly generated verilog back into the board's source, run:
